@echo off
rem RepoLucent double-click launcher (Windows).
rem Usage:
rem   start_repolucent.bat                     -> repo taken from repolucent.ini (repo=...) or %1
rem   start_repolucent.bat D:\path\to\repo     -> analyze given repo root
rem Behavior:
...
rem --- resolve repo: %1 overrides repolucent.ini ---
set "REPO="
if not "%~1"=="" set "REPO=%~1"
if not defined REPO (
...
if not defined REPO (
    echo [RepoLucent] 未指定仓库：请在 repolucent.ini 设置 repo= 或传入路径参数。
    pause
    exit /b 1
)
