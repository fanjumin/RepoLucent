...
作为库导入：
    from repo_lucent.scriptlib.subprocess_harness import JsonRpcProc, roundtrip, expect
    proc = JsonRpcProc([sys.executable, "server.py"])
    ok = roundtrip(proc, [("initialize", {}, expect.has_result), ...])

CLI（跑内置 demo，证明往返机制可用）：
    python -m repo_lucent.scriptlib.subprocess_harness --demo
    repolucent.py script run subprocess_harness
