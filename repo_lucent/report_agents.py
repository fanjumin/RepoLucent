...
本模块把 repo_lucent.json 里**已有**的事实（overview / core / plugins / standards /
...
3. **标记匹配与品牌解耦**：标记块的正则只依赖稳定的 `:begin auto` / `:end auto` 词形，
   不绑定 "repolucent" 字样——v2.0.0 更名后，旧仓库里既有的标记块仍能被识别与更新。
...
_L = {
    "zh": {
        "title": "# AGENTS.md",
        "note": "> 本文件由 RepoLucent 自动生成的标记块维护；块外内容为仓库自有约定，"
                "不随工具运行变化。",
        "overview": "项目速览",
        "contract": "组件契约",
        "manifest": "清单必填字段与枚举",
    ...
    "en": {
        "title": "# AGENTS.md",
        "note": "> Maintained by a RepoLucent-generated marker block; everything outside "
...
def render_agents_block(data: dict, cfg=None) -> str:
    ...
    w("repolucent --repo <repo>")
    w("")
    w("# 只打印关键指标（stdout key=value，不落盘）")
    w("repolucent --repo <repo> --summary-only")
    w("")
    if gate_items:
    ...
    w("---")
    w(f"*block generated by RepoLucent v{TOOL_VERSION}; "
