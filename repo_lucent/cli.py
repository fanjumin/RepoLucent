...
用法：
    ...
    python repolucent.py snapshot --save baseline  # 生成基线快照（out/history/baseline.json）
    python repolucent.py diff baseline             # 与基线对比，输出变更报告
    python repolucent.py diff baseline --format json --top 20
    python repolucent.py search parse_python_file  # 符号定位（file:line / kind / owner）
"""
...
def _pack_disabled_text(cmd: str, pack: str) -> str:
    ...
    return (f"[repolucent] 错误：命令 `{cmd}` 属于 pack `{pack}`，当前未启用。\n"
            f"  当前生效 pack：{cur} ｜ 可用 pack：{avail}\n"
            f"  启用方法：在 settings.json 中设置 "
            f"\"packs\": {{\"enabled\": [\"{pack}\"]}}（或用 REPO_LUCENT_SETTINGS "
...
def _add_shared_args(ap: argparse.ArgumentParser) -> None:
    ...
    ap.add_argument("--repo-name", default=argparse.SUPPRESS, metavar="NAME",
                    help="使用多仓库注册表（repolucent repos add 注册）中的仓库；"
...
def _run_gates(args, data: dict, cfg, stream) -> int:
    ...
    try:
        ...
        print(f"[repolucent] 错误：{e}", file=sys.stderr)
        return 2
...
#   * 失效信号：TTL 安全网（默认 15s，可用环境变量 REPO_LUCENT_ANALYSIS_TTL 调整）
...
#: **刻意不写入任何产物**——冷跑与热跑的该值必然不同，进入 repo_lucent.json
...
def _analysis_freshness(cfg: "RepoConfig"):
    """失效 token：仓库身份 + TTL 桶（默认 15s，可用 REPO_LUCENT_ANALYSIS_TTL 调整）。

    仓内容变化不靠轮询，而由显式写操作调用 invalidate_analysis_cache() 立即失效；
    TTL 仅作安全网，避免「用户改完代码却未点重新分析」时无限期陈旧。
    """
    try:
        ttl = max(float(os.environ.get("REPO_LUCENT_ANALYSIS_TTL")
...
def _analyze_compute(args, cfg: RepoConfig,
    ...
    if hs:
        ...
        # 刻意不写进事实源：同一输出目录二次运行会得到不同取值，进入 repo_lucent.json
    ...
    # 写入独立产物 repo_lucent_symbols.json，避免继续撑大唯一事实源。
...
def _cmd_analyze(args) -> int:
    ...
    if getattr(args, "module", None) and getattr(args, "plugin", None):
        print("[repolucent] 错误：--module 与 --plugin 不能同时使用", file=sys.stderr)
        return 2
    cfg = _setup(args, with_target=True)
    if not args.quiet and not args.summary_only:
        print(f"[repolucent] 仓库: {cfg.repo_root}")
    ...
    if cfg.target:
        print(f"[repolucent] 指定分析：{cfg.target_type} `{cfg.target}` → "
    ...
    if not args.quiet:
        ...
        print(f"[repolucent] 完成：核心模块 {data['core']['module_count']} 个 · "
    ...
    if Path(cfg.repo_root) / "AGENTS.md" in written:
        print("[repolucent] 注意：AGENTS.md 已写入仓库根（agents_md=repo），"
...
def _cmd_repos(args) -> int:
    """多仓库注册表管理（本地 ~/.repolucent/repos.json）。"""
    ...
    if sub == "remove":
        if not args.confirm:
            print(f"[repolucent] 预览：将移除注册项 {args.name}（不影响磁盘文件）。加 --confirm 执行。")
            return 0
        ok = remove_repo(args.name)
        print(f"[repolucent] {'已移除' if ok else '未找到'}：{args.name}")
...
def _cmd_diff(args) -> int:
    ...
    try:
        ...
        if avail:
            ...
            print("  尚无基线，请先运行：repolucent.py snapshot --save baseline",
                  file=sys.stderr)
        return 2
    except ValueError as e:
        print(f"[repolucent] 错误：{e}", file=sys.stderr)
...
def build_search(cfg: RepoConfig, data: dict, symbol: str, limit: int = 50,
    ...
    if not index.get("count"):
        res["note"] = ("符号索引为空：请先执行一次全量分析（repolucent --repo <repo>）"
...
def _cmd_context(args) -> int:
    ...
    try:
        ...
        print("  用法：repolucent context --for plugin:<identifier> "
...
def _cmd_log(args) -> int:
    ...
    if "error" in data:
        print(f"[repolucent] 错误: {data['error']}", file=sys.stderr)
        return 2
...
def _cmd_group(args) -> int:
    ...
    if group_cmd == "add":
        ...
        print(f"[repolucent] 已将仓库添加到组 '{args.group_name}': {repo_path}")
        if remotes:
        ...
        if success:
            print(f"[repolucent] 已从组 '{args.group_name}' 移除仓库: {repo_path}")
            return 0
        else:
            print(f"[repolucent] 错误: 仓库不在组 '{args.group_name}' 中", file=sys.stderr)
            return 2

    else:
        print("[repolucent] 错误: 请指定子命令 (add/list/remove)", file=sys.stderr)
        return 2
...
def _cmd_batch(args) -> int:
    ...
    if group_name not in groups:
        print(f"[repolucent] 错误: 仓库组 '{group_name}' 不存在", file=sys.stderr)
...
def _cmd_push(args) -> int:
    ...
    if "error" in result:
        print(f"[repolucent] 错误: {result['error']}", file=sys.stderr)
...
def _cmd_sync(args) -> int:
    ...
    if len(results) == 1 and "error" in results[0] and len(results[0]) == 1:
        print(f"[repolucent] 错误: {results[0]['error']}", file=sys.stderr)
        return 2
...
def _cmd_mcp_out(args) -> int:
    ...
    if not is_outbound_mcp_enabled():
        print("[repolucent] outbound MCP 已关闭（settings: mcp_outbound_enabled=false）",
              file=sys.stderr)
        return 2
    ...
    if action == "servers":
        ...
        if not servers:
            print("（未配置外部 MCP Server；在 settings.json 的 mcp_servers 中声明，"
                  "或用 REPO_LUCENT_SETTINGS 指向一份配置）")
            return 0
        ...
        print(f"\n共 {len(servers)} 个；发现工具：repolucent.py mcp-out tools --server <NAME>")
        return 0

    # ---- tools：发现外部工具 ----
    if action == "tools":
        p = argparse.ArgumentParser(prog="repolucent.py mcp-out tools", add_help=False)
        ...
        if opts.as_json:
            ...
            print(f"\n共 {len(tools)} 个外部工具；调用：repolucent.py mcp-out call <server> <tool> --confirm")
        return 0

    # ---- call：调用某个外部工具 ----
    if action == "call":
        p = argparse.ArgumentParser(prog="repolucent.py mcp-out call", add_help=False)
        ...
        if len(pos) < 2:
            print("[repolucent] 用法：mcp-out call <server> <tool> [--arg k=v ...] "
        ...
        for kv in opts.arg:
            if "=" not in kv:
                print(f"[repolucent] --arg 须为 k=v 形式：{kv}", file=sys.stderr)
                return 2
            k, v = kv.split("=", 1)
            arguments[k] = v
        if not opts.confirm:
            print("[repolucent] dry-run（未 --confirm）：")
        ...
        try:
            ...
            print(f"[repolucent] 外部 MCP 调用失败：{e}", file=sys.stderr)
            return 3
        except Exception as e:  # noqa: BLE001 - 外部服务异常一律收敛为 rc=3
            print(f"[repolucent] 外部 MCP 调用异常：{type(e).__name__}: {e}", file=sys.stderr)
            return 3
        if opts.as_json:
    ...
    print("[repolucent] 用法：mcp-out {servers|tools|call} ...", file=sys.stderr)
    return 2
...
def _cmd_audit(args) -> int:
    ...
    if getattr(args, "audit_rules", None):
        try:
            ...
            print(f"[repolucent] 错误：无法解析 --audit-rules：{e}", file=sys.stderr)
            return 2
    ...
    if getattr(args, "semantic_results", None):
        try:
            ...
            print(f"[repolucent] 错误：无法解析 --semantic-results：{e}", file=sys.stderr)
            return 2
        for r in rejects:
    ...
    if getattr(args, "diff_baseline", None):
        ...
        if base is None:
            print(f"[repolucent] 找不到审计基线：{args.diff_baseline}"
...
def _clean_message(msg: str) -> str:
    ...
    return re.sub(r"^\[(?:repolucent|dev-insight)\]\s*", "", text)
...
def _emit_error(etype: str, message: str, hint: str) -> None:
    ...
    try:
        ...
        print(f"[repolucent] 运行失败: {etype}: {message}", file=sys.stderr)
    ...
    try:
        ...
        if isinstance(code, str):
            ...
            _emit_error("ArgumentError", f"命令行参数错误（exit {code}）",
                        "用 `repolucent.py --help` 查看用法；子命令用 `repolucent.py <cmd> --help`。")
        return 2                                  # 2 = 参数/环境错误
