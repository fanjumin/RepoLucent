...
#: 工具版本号统一由 repo_lens/__init__.py 的 TOOL_VERSION 提供（单一事实源），
...
#   * 开关：settings.output.date_dir（默认 true）> 环境变量 REPO_LENS_DATE_DIR
...
def date_dir_setting() -> tuple[bool, str]:
    ...
    env = os.environ.get("REPO_LENS_DATE_DIR") or os.environ.get("VR_INSIGHT_DATE_DIR")
    if env is not None:
...
#                      属有意行为，必须显式选择）；
#   off             —— 不生成。
# 解析优先级：CLI --agents-md > 环境变量 REPO_LENS_AGENTS_MD > settings.output.agents_md
...
def agents_md_setting() -> str:
    ...
    env = (os.environ.get("REPO_LENS_AGENTS_MD")
