...
def render_ai_context(data: dict, max_plugins: int = 60) -> str:
    ...
    w("# 仓库开发上下文（RepoLens 自动生成）")
    w("")
    w(f"> 生成：RepoLens v{TOOL_VERSION} · {data['meta']['generated_at']} · "
    ...
    if std.get("plugin_standard"):
        ...
        w("")
    w("---")
    w("*本地工具生成物，不进入 Git。完整信息见 repo_lens_report.md / repo_lens.json。*")
    return "\n".join(L) + "\n"
...
def render_target_context(data: dict, dd: dict, depth: str = "brief") -> str:
    ...
    w("")
    w(f"> RepoLens v{TOOL_VERSION} · 仓库 `{data['meta']['repo_root']}` · "
      f"depth={depth}")
    w("")
