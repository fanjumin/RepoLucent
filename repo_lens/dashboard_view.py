...
def render_dashboard(d: dict) -> str:
    ...
    *{{box-sizing:border-box}}
    ...
    <div class="layout">
    <aside class="side">
    <div class="logo"><i></i>RepoLens</div>
    <div class="nav">
    ...
    <div style="margin-top:1rem;font-size:.68rem;opacity:.5;padding:0 .5rem">RepoLens v{ver}<br>{esc(gen)}</div>
    </aside>
    <main class="main" id="top">
    <div class="head"><div><h1>仓库架构仪表盘<span class="badge">v{ver}</span></h1>
    <div class="sub">主仓库 ＋ 前端/扩展仓库 · 数据由 RepoLens 自动采集 · 本地辅助工具，不入 Git</div></div></div>
    ...
    <div class="foot"><span>完整能力：控制台 http://127.0.0.1:8788 · 报告 repo_lens_report.html · AI 上下文 AI_CONTEXT.md</span><span>RepoLens v{ver} · schema {d['meta']['schema_version']}</span></div>
    </main></div></body></html>"""
    return html
