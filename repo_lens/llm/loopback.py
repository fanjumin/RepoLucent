...
`python -m repo_lens mcp` 子进程，用现成的 `JsonRpcProc` 走标准 tools/call。
...
_TOOL_ROOT = Path(__file__).resolve().parent.parent.parent  # 仓库根（repo_lens 的父目录）
...
class McpLoopback:
    ...
    def start(self) -> None:
        env = dict(os.environ)
        # 让子进程可导入 repo_lens（等价于「已安装」场景，但不写死本工具绝对路径）
        env["PYTHONPATH"] = str(_TOOL_ROOT) + os.pathsep + env.get("PYTHONPATH", "")
        env.setdefault("PYTHONIOENCODING", "utf-8")
        proc = JsonRpcProc(
            [self.python_exe, "-m", "repo_lens", "mcp",
