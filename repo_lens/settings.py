...
_HERE = Path(__file__).resolve().parent          # repo_lens/
...
def _search_paths() -> list[Path]:
    ...
    env = os.environ.get("REPO_LENS_SETTINGS") or os.environ.get("VR_INSIGHT_SETTINGS")
    if env:
...
def get_secret(key: str, default: str | None = None) -> str | None:
    """密钥只从环境变量读取（如 REPO_LENS_DASHSCOPE_KEY）。绝不经 settings.json。

    这是 P2 LLM provider 的密钥读取通道：get_secret(model["key_env"])。
    settings.json 只允许用 key_env / base_url_env 指向环境变量，不得出现明文密钥。

    过渡期兼容：REPO_LENS_ 前缀密钥未设置时回落读取旧 VR_INSIGHT_ 同名变量。
    """
    v = os.environ.get(key)
    if v is None and key.startswith("REPO_LENS_"):
        v = os.environ.get("VR_INSIGHT_" + key[len("REPO_LENS_"):])
...
def resolve_profile_source(name: str | None):
    ...
    搜索顺序：<tool>/profiles/<name>.json → ~/.repolens/profiles/<name>.json。
    """
