...
用途：Agent 与 CI 在消费 repo_lens.json 前做一次机器可校验的兼容性检查
（Google Tricorder 式「产物契约 + 持续验证」的最小闭环）。

设计边界：
- 不做远程 `$ref`、不做 `allOf/anyOf/oneOf/patternProperties` 等组合关键字；
  契约文件本身也不使用它们（见 schemas/repo_lens.schema.json）。
...
#: 契约文件位置（随包分发）
SCHEMA_PATH = Path(__file__).resolve().parent / "schemas" / "repo_lens.schema.json"
...
def validate_report(data, path=None) -> list[str]:
    """按随包契约校验一份 repo_lens.json 载荷。"""
    return validate(data, load_schema(path))


def main(argv=None) -> int:
    """CLI：python -m repo_lens.schema_check <repo_lens.json> [schema.json]"""
    import sys
    args = list(argv if argv is not None else sys.argv[1:])
    if not args:
        print("用法：python -m repo_lens.schema_check <repo_lens.json> [schema.json]")
        return 2
    target = Path(args[0])
    try:
