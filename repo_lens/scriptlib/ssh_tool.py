...
- 凭证只来自环境变量：REPO_LENS_SSH_USER / REPO_LENS_SSH_PASS
  （兼容旧名 VR_SSH_USER / VR_SSH_PASS）；支持 --key 指定私钥文件（口令同样走 REPO_LENS_SSH_PASS）。
...
用法：
    python -m repo_lens.scriptlib.ssh_tool --host 10.0.0.2 --user root \
        --cmd "uname -a" --cmd "df -h /" --json
    python -m repo_lens.scriptlib.ssh_tool --host h --cmd "systemctl restart nginx" --yes-i-know
...
def _creds():
    user = os.environ.get("REPO_LENS_SSH_USER") or os.environ.get("VR_SSH_USER")
    pwd = os.environ.get("REPO_LENS_SSH_PASS") or os.environ.get("VR_SSH_PASS")
    return user, pwd
...
def main(argv: list[str] | None = None) -> int:
    ...
    pwd = env_pwd
    if not user:
        print("[ssh_tool] 缺少用户：传 --user 或设置 REPO_LENS_SSH_USER", file=sys.stderr)
        return 2
    if not args.key and not pwd:
        print("[ssh_tool] 缺少凭证：设置 REPO_LENS_SSH_PASS（或旧名 VR_SSH_PASS），"
