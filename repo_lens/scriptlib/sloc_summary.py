...
注意：与 repo_lens 主命令的 overview LOC 口径不同（那是 VeroRun 全仓、含本地脚本排除规则）；
本脚本是通用速查，勿与主命令统计混用（overlap）。

复用工具排除知识（config.DEFAULT_EXCLUDE_DIRS），独立运行时优雅降级。

用法：
    python -m repo_lens.scriptlib.sloc_summary --root <目录> [--top 15] [--json] [--out out.json]
    repolens.py script run sloc_summary --root <目录>
