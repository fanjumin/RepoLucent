# -*- coding: utf-8 -*-
"""本地可视化控制台（`repolens.py serve`）：给人点的界面 + 给 Agent 调用的 HTTP API。
...
安全模型（v1.5.1 起，四层递进，参考 Jupyter Server token 与 Chrome DevTools host 校验）：
    ...
    L3 Token 鉴权    /api/* 与 /mcp 必须 Bearer token 或 X-RepoLens-Token 头；
                     token 由 secrets.token_urlsafe(24) 生成，可用环境变量
                     REPOLENS_TOKEN 固定（供 MCP 客户端配置复用）；恒时比较。
                     显式设置 REPOLENS_TOKEN=""（空串）= 退回无鉴权模式（仅本机自担风险，
                     启动横幅红色警告）。
...
_TOKEN_PLACEHOLDER = "__REPOLENS_TOKEN__"
_VERSION_PLACEHOLDER = "__REPOLENS_VERSION__"


def resolve_auth_token() -> tuple[str, bool]:
    """解析控制台访问令牌。返回 (token, degraded)。

    - 环境变量 REPOLENS_TOKEN 非空 → 固定 token（MCP 客户端可配置复用）；
    - 显式设置 REPOLENS_TOKEN=""（空串）→ 降级为无鉴权模式（degraded=True，
      仅本机自担风险，启动横幅红色警告）；
    - 未设置 → secrets.token_urlsafe(24) 随机生成（默认）。
    """
    env = os.environ.get("REPOLENS_TOKEN")
    if env is None:
...
# P1a UI 侧栏化：控制台静态资源外置到 repo_lens/ui/static，由 /static 服务。
...
class _Handler(BaseHTTPRequestHandler):
    server_version = f"RepoLens/{TOOL_VERSION}"
    state: dict = {}
    ...
    def _guard(self) -> bool:
        ...
        if needs_token and self.server.auth_token:
            ...
            if tok.startswith("Bearer "):
                tok = tok[len("Bearer "):]
            tok = tok.strip() or (self.headers.get("X-RepoLens-Token") or "").strip()
...
def _api_state(h, q, body):
    ...
    out = cfg.out_dir
    report = out / "repo_lens_report.html"
    return _ok({
...
def _api_data(h, q, body):
    data_file = h.state["cfg"].out_dir / "repo_lens.json"
...
def _api_report(h, q, body):
    report = h.state["cfg"].out_dir / "repo_lens_report.html"
...
def start(cfg: RepoConfig, frontends: list, analyzer, summarize,
    ...
    if _port_in_use(host, port):
        print(f"[repolens] 错误：端口 {port} 已被占用——可能有上一次或旧版本的服务仍在运行。",
              file=sys.stderr)
        print(f"[repolens] 提示：该端口上的服务未必是本版本；旧进程会导致前端静态资源 404、页面卡住。",
              file=sys.stderr)
        print(f"[repolens]   排查（Windows）：netstat -ano | findstr :{port}"
              f"，再 taskkill /PID <pid> /F", file=sys.stderr)
        print(f"[repolens]   或改用其他端口：repolens.py serve --port {port + 1}",
              file=sys.stderr)
        raise SystemExit(2)
    ...
    try:
        ...
        print(f"[repolens] 错误：无法绑定 {host}:{port} —— {e}", file=sys.stderr)
        raise SystemExit(2) from None
    print(f"[repolens] 控制台已启动：http://{host}:{port}")
    print(f"[repolens] 仓库：{cfg.repo_root} · 输出：{cfg.out_dir} · Ctrl+C 退出")
    if degraded:
        # 显式降级：REPOLENS_TOKEN="" 选择了无鉴权模式（仅本机自担风险）
        print("\033[91m[repolens] 警告：REPOLENS_TOKEN 为空——本服务已退回无鉴权模式，"
              "本机任何进程/页面都可调用全部 API。\033[0m", file=sys.stderr)
        print("[repolens] 警告：如需恢复鉴权，请删除空的 REPOLENS_TOKEN 环境变量后重启。",
              file=sys.stderr)
    else:
        print(f"[repolens] 访问令牌: {token}")
        if os.environ.get("REPOLENS_TOKEN"):
            print("[repolens] （token 来自 REPOLENS_TOKEN 环境变量，MCP 客户端可固定复用）")
        print(f"[repolens] MCP 客户端请配置 header: X-RepoLens-Token: <token>"
              f"（浏览器打开首页会自动注入）")
    try:
        srv.serve_forever()
...
<title>RepoLens 控制台</title>
<link rel="stylesheet" href="/static/tokens.css">
<script>window.__REPOLENS_TOKEN="__REPOLENS_TOKEN__";</script>
</head>
<body>
<div class="scr">
  <div class="tbar">
    <span class="tbd"><span class="vmark" style="width:15px;height:15px;border-radius:4px;font-size:9px">V</span>RepoLens</span>
    <span>本地控制台 · 仅绑定 127.0.0.1</span>
    <span class="winv"><span>—</span><span>□</span><span>?</span></span>
  </div>
  <div class="hdr">
    <div class="brand"><span class="vmark">R</span><span><span class="bname">RepoLens</span><br><span class="bsub">CODE LENS</span></span><span class="edchip">v__REPOLENS_VERSION__</span></div>
  ...
  <div class="sbar">
    ...
    <span class="sgroup mono">REPOLENS · v__REPOLENS_VERSION__</span>
  </div>
</div>
