# -*- coding: utf-8 -*-
"""`repolens.py script` 子命令：脚本资产库的编目 / 调用 / 自检。
...
中 entry 非空、且位于 ``repo_lens.scriptlib`` / ``repo_lens.packs`` 下的模块；
...
def _load_core_registry() -> dict:
    ...
    for p in (_CORE_REGISTRY_PATH, _LEGACY_REGISTRY_PATH):
        ...
        try:
            ...
            print(f"[repolens] 错误：注册表 JSON 解析失败：{e}", file=sys.stderr)
            return {"scripts": []}
    print(f"[repolens] 错误：注册表缺失 {_CORE_REGISTRY_PATH}", file=sys.stderr)
    return {"scripts": []}
...
def _print_table(scripts: list[dict]) -> None:
    ...
    print(f"\n共 {len(scripts)} 条；运行：repolens.py script run <id> [-h 查参数]；"
          f"planned/archived 尚不可运行。")


def _act_list(args) -> int:
    # 主解析器用 REMAINDER 捕获 list 之后的所有 token，故本地再解析这些可选过滤参数
    p = argparse.ArgumentParser(prog="repolens.py script list", add_help=False)
...
def _act_info(args) -> int:
    ...
    if not s:
        print(f"[repolens] 错误：未找到脚本 id={sid!r}；用 `script list` 查看全部。",
              file=sys.stderr)
        return 2
...
def _act_run(args) -> int:
    ...
    if not rest:
        print("[repolens] 错误：script run 需要 <id>。", file=sys.stderr)
        return 2
    sid, script_argv = rest[0], rest[1:]
    reg = _load_registry()
    s = _index(reg).get(sid)
    if not s:
        print(f"[repolens] 错误：脚本 id={sid!r} 不在注册表白名单内。", file=sys.stderr)
        return 2
    entry = s.get("entry")
    if not entry:
        st = s.get("status")
        print(f"[repolens] 脚本 {sid} 状态={st}，尚不可运行。"
              f"{'（planned：阶段 B/C 落地后启用）' if st=='planned' else '（archived：一次性归档，仅溯源）'}",
              file=sys.stderr)
        return 2
    if not any(entry.startswith(p) for p in _ALLOWED_PKGS):
        print(f"[repolens] 拒绝：脚本 entry={entry} 不在允许包 "
              f"{' / '.join(_ALLOWED_PKGS)} 下（安全边界）。", file=sys.stderr)
        return 3
    try:
        mod = importlib.import_module(entry)
    except Exception as e:  # noqa: BLE001
        print(f"[repolens] 导入脚本失败：{type(e).__name__}: {e}", file=sys.stderr)
        return 3
    main = getattr(mod, "main", None)
    if not callable(main):
        print(f"[repolens] 脚本 {sid} 无 main(argv) 入口。", file=sys.stderr)
        return 3
...
def run_script_api(sid: str | None, argv: list[str] | None = None,
    ...
    cmd_preview = f"repolens.py script run {sid} " + " ".join(argv)
    if not confirm:
...
def _render_manual(reg: dict) -> str:
    ...
    L.append("> 本手册由 `repolens.py script manual` 从 `repo_lens/registry.core.json` "
             "与各启用 pack 的 `repo_lens/packs/<name>/registry.json` 自动生成，"
    ...
    L.append("## 快速上手")
    L.append("")
    L.append("```bash\npython repolens.py script list\n"
             "python repolens.py script info <id>\n"
             "python repolens.py script run <id> [脚本参数...]\n"
             "python repolens.py script doctor\n```")
    L.append("")
...
def _build_script_argparser() -> argparse.ArgumentParser:
    ...
    return argparse.ArgumentParser(prog="repolens.py script", add_help=True)
...
# 供独立调试（不走 cli.py）：python -m repo_lens.script_cmd script list
